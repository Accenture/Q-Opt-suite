...
class QuantagoniaQuboCallback(ToQuboCallback):
    ...
    def set_num_variables(self, num_variables: int) -> None:
        self.vars = [self.qubo.addVariable(name=str(v)) for v in range(num_variables)]
...
class Quantagonia(Platform):
    ...
    def translate_problem(self, step: ModelStep) -> qq.QuboModel:
        ...
        """
        callback = QuantagoniaQuboCallback()
        self.construct_qubo(step, callback)
        return callback.qubo
    ...
    def num_variables(self, problem: Any) -> int:
        ...
        """
        qubo_model = cast(qq.QuboModel, problem)
        return len(qubo_model.vars)
    ...
    def solve(
        ...
        qubo_model.solve(spec.getd(), runner=self._runner)
        return qubo_model
    ...
    def translate_result(
        ...
        """
        assert num_solutions_desired == 1  # TODO implement multiple result support
        qubo_model = cast(qq.QuboModel, result)

        result = [
            qubo_model.vars[str(v)].eval()
